...
public class SendSSLEmailActivity extends BaseActivity implements View.OnClickListener {

    private static final String TAG = SendSSLEmailActivity.class.getSimpleName();

    public static void invoke(Context context) {
        Intent intent = new Intent(context, SendSSLEmailActivity.class);
    ...
    private void sendEmail() {
        ...
        if (TextUtils.isEmpty(content.getText().toString().trim())) {
            mailInfo.setContent(EmailConfiguration.getEmailBody(SendSSLEmailActivity.this, fromEmail.getText().toString().trim(), ""));
        } else {
            mailInfo.setContent(EmailConfiguration.getEmailBody(SendSSLEmailActivity.this, fromEmail.getText().toString().trim(), content.getText().toString().trim()));
        }
    ...
    class Task implements Runnable {
        ...
        public void run() {
            try {
                ...
                SimpleMailSender.sendSSLTextMail(mailInfo);
            } catch (Exception e) {
