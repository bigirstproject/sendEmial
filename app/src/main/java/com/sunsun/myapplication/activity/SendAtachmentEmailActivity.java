...
public class SendAtachmentEmailActivity extends BaseActivity implements View.OnClickListener {

    private static final String TAG = SendAtachmentEmailActivity.class.getSimpleName();

    public static void invoke(Context context) {
        Intent intent = new Intent(context, SendAtachmentEmailActivity.class);
    ...
    private void sendEmail() {
        ...
        if (TextUtils.isEmpty(content.getText().toString().trim())) {
            mailInfo.setContent(EmailConfiguration.getEmailBody(SendAtachmentEmailActivity.this, fromEmail.getText().toString().trim(), ""));
        } else {
            mailInfo.setContent(EmailConfiguration.getEmailBody(SendAtachmentEmailActivity.this, fromEmail.getText().toString().trim(), content.getText().toString().trim()));
        }
    ...
    class Task implements Runnable {
        ...
        public void run() {
            try {
                ...
                SimpleMailSender.sendAtachmentTextMail(mailInfo);
            } catch (Exception e) {
